...
### Added
- **Antigravity CLI (`agy`) support**: `agy` is now a first-class fourth `cli`, alongside `claude`, `codex`, and `gemini`. It is Google's official successor to Gemini CLI for individual Google AI Pro/Ultra accounts. Two of its quirks are handled transparently by the runner: it has **no JSON output mode** (LimitShift reads its plain-text stdout — captured separately from stderr so a trailing diagnostic line can't hide the completion marker), and it has **no per-conversation session ids** (resume continues the most recent conversation with `agy -c`, driven by a sentinel session marker, so agy tasks are inherently sequential). The prompt is passed as the value of `-p` (agy does not read stdin); `--model` is passed through; there is no `effort` flag (validation requires `null`) and no Ollama path. The permission flag is `--dangerously-skip-permissions`. On Windows, native-exe arguments are now delivered as a single canonical command-line string, so agy's whole multi-line prompt survives `CreateProcess` intact.
